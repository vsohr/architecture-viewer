...
// example architecture rendered by Arch Viewer.
//
// Top-level system + two drilled-in subsystems (orchestrator, signals)
// + flat edge list + comment annotations + a sample validation error.
...
const TRADERANK = {
  ...
  // Each level has its own nodes + edges. Children of a node are
  // the nodes that appear when you drill into that node.
  levels: {
    traderank: {
      bounds: { w: 1480, h: 760 },
      nodes: [
  ...
  // Comments are HTML-comment-anchored marginalia. They reference a
  // (level-id, node-id) pair.
  comments: [
    { id: "c1", levelId: "traderank", nodeId: "orchestrator", author: "vlad", date: "2026-04-22",
    ...
    { id: "c2", levelId: "traderank", nodeId: "tickdb", author: "vlad", date: "2026-03-08",
      body: "InfluxDB 2.x. 90 days hot, then archived to S3 glacier. Compaction Sundays 02:00 UTC — expect query latency spikes." },
    { id: "c3", levelId: "traderank", nodeId: "broker", author: "miriam", date: "2026-04-30",
    ...
    { id: "c4", levelId: "traderank.orchestrator", nodeId: "executor", author: "vlad", date: "2026-04-22",
    ...
    { id: "c5", levelId: "traderank.orchestrator", nodeId: "limits", author: "miriam", date: "2026-02-14",
      body: "Pure crate, no I/O, no clock. If a test needs the wall-clock, you're in the wrong file." }
  ],

  // A sample structured validation error to show the error panel state.
  sampleErrors: [
    { file: "ARCHITECTURE.md", line: 47, col: 12,
      message: "edge `orchestrator.legacy_executor → broker` references undeclared node `orchestrator.legacy_executor`",
      hint: "Was this renamed to `executor`? Add the node, or update the edge." },
    { file: "ARCHITECTURE.md", line: 62, col: 9,
